...
class Conv2dShift(_ConvNdShift):
    ...
    def forward(self, input, ratio_out=1, ratio_in=1, ratio_g=1, kernel=None):
        ...
        # if True:
            ...
            sample_weight = weight_ps[:(self.out_channels//ratio_out),:(self.in_channels//ratio_in),:,:]
            if (kernel!=None):
                ...
                padding = self.padding
            output = torch.nn.functional.conv2d(input_padded, sample_weight, bias_fixed_point,
                                              self.stride, padding, self.dilation, (self.groups//ratio_g))
