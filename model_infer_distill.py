...
class FBNet_Infer(nn.Module):
    def __init__(self, alpha, config, flag=False, cand=None):
        ...
        self.linear_trans = nn.ModuleList()
            ...
            for i in range(num_layer):
                if i == 0:
                    if stage_id == 0:
                        ...
                        linear = nn.Conv2d(self.num_channel_list[stage_id], self.num_channel_list[stage_id],kernel_size=1, stride=1, padding=0, dilation=1, groups=1, bias=False)
                    else:
                ...
                self.linear_trans.append(linear)
        ...
        self._var_params = self._build_var_parameters()
        self._reset_var_parameters()
    ...
    def _build_var_parameters(self):
        # num_ops = len(self.linear_trans)
        setattr(self, 'sita', nn.Parameter(Variable(torch.ones(sum(self.num_layer_list)).cuda(), requires_grad=True)))

        return {"sita": self.sita}


    def _reset_var_parameters(self):
        # num_ops = len(self.linear_trans)
        getattr(self, "sita").data = Variable(torch.ones(sum(self.num_layer_list)).cuda(), requires_grad=True)
    ...
    def forward(self, input, flag=False):
        ...
        middle_out = []
        feature = []
        sita = getattr(self, "sita")

        for i, (cell,linear) in enumerate(zip(self.cells, self.linear_trans)):
            out = cell(out)
            # feature.append(out)
            # if i == 4 or i == 8 or i == 12 or i == 16 or i == 20:
            #     middle_out.append(out)
            if flag == True:
                feature = torch.exp(-out/2*sita[i]*sita[i])
            else:
                if (self.op_idx_list[i] < 6) or (self.op_idx_list[i] == 12):
                    feature = torch.exp(-out/2*sita[i]*sita[i])
                else:
                    feature = torch.exp(-out/sita[i])
            
            feature = linear(feature)
            middle_out.append(feature)
