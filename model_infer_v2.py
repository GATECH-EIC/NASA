    ...
    def __init__(self, alpha, config, flag=False, cand=None):
        ...
        for stage_id, num_layer in enumerate(self.num_layer_list):
            for i in range(num_layer):
                ...
                if i == 0:
                    if stage_id == 0:
                        op = MixedOp(self.stem_channel, self.num_channel_list[stage_id], self.op_idx_list[cout], layer_id, stride=self.stride_list[stage_id], search_space=self.search_space, flag=flag)
                    else:
                        op = MixedOp(self.num_channel_list[stage_id-1], self.num_channel_list[stage_id], self.op_idx_list[cout], layer_id, stride=self.stride_list[stage_id], search_space=self.search_space, flag=flag)
                else:
                    op = MixedOp(self.num_channel_list[stage_id], self.num_channel_list[stage_id], self.op_idx_list[cout], layer_id, stride=1, search_space=self.search_space, flag=flag)
                cout += 1
                layer_id += 1
