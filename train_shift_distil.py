...
def main():
    ...
    if args.transfer_epoch is not None:
        config.transfer_epoch = args.transfer_epoch
...
def main_worker(gpu, ngpus_per_node, config):
    ...
    if (config.spos==True):
        ...
        if config.distillation == True:
            ...
            flops = model_teacher.forward_flops((3, config.image_height, config.image_width))
            print('FLOPs: ', flops)
            # print(model_teacher)
    ...
    if config.distillation == True:
        if type(pretrain) == str and os.path.exists(pretrain + "/teacher_weights_best_%d.pth" %(config.load_epoch)):
            pretrained_model = torch.load(pretrain + "/teacher_weights_best_%d.pth" %(config.load_epoch))
            partial = pretrained_model['state_dict']
            # print("ckpt:",partial)
            ...
            start_epoch = pretrained_model['best_epoch']+1
            # best_epoch = pretrained_model['best_epoch']
    ...
    for epoch in range(start_epoch, config.nepochs):
        ...
        if config.distillation == True and epoch == (config.transfer_epoch): 
            pretrained_model = torch.load(config.save + '/teacher_weights_%d.pth' %(config.transfer_epoch))
            partial = pretrained_model['state_dict']
            # print("ckpt:",partial)
            state = model.state_dict()
            pretrained_dict = {k: v for k, v in partial.items() if k in state and state[k].size() == partial[k].size()}
            state.update(pretrained_dict)
            model.load_state_dict(state)
            print("Teacher transferred!!!!!!")

        if config.distillation == True:
            if epoch < config.transfer_epoch:
                train(train_loader, model_teacher, optimizer_teacher, lr_policy, logger, epoch, config)
                print("Teacher is training!!!!!!!!!!")
        ...
        if (config.distillation == False) or (config.distillation == True and (not epoch < config.transfer_epoch)):
            if acc > best_acc:
                best_acc = acc
                best_epoch = epoch
                state = {}
        ...
        if config.distillation == True:
            if acc_teacher > best_acc_teacher:
                ...
                state_teacher['best_acc'] = best_acc_teacher
                state_teacher['best_epoch'] = best_epoch_teacher
                torch.save(state_teacher, os.path.join(config.save, 'teacher_weights_best_%d.pth' %(Epoch+1)))

        if config.distillation == True and epoch == (config.transfer_epoch-1):
            # if acc_teacher > best_acc_teacher:
            # best_acc_teacher = acc_teacher
            # best_epoch_teacher = epoch
            state_teacher = {}
            ...
            # state_teacher['best_acc'] = best_acc_teacher
            # state_teacher['best_epoch'] = best_epoch_teacher
            torch.save(state_teacher, os.path.join(config.save, 'teacher_weights_%d.pth' %(epoch+1)))
...
def train(train_loader, model, optimizer, lr_policy, logger, epoch, config, model_tearcher=None, optimizer_teacher=None):
    # global distil_loss
    model.train()
    if config.distillation == True and (not epoch < config.transfer_epoch):
    ...
    for step, (input, target) in enumerate(train_loader):
        ...
        if config.distillation == True and (not epoch < config.transfer_epoch):
            ...
            out_student_2 = F.log_softmax(out_student/T, dim=1)
            out_teacher_2 = F.softmax(out_teacher_2/T, dim=1)
            # print('outputs_main',outputs_main)
            distillation_loss = kl_div(out_student_2, out_teacher_2) * T * T
            with torch.no_grad():
                middle_teacher_2 = []
