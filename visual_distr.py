...
fig, ax = plt.subplots(8,1,figsize=(10, 25))
pretrained_model = torch.load('/Datadisk/shihh/NAS/baseline/CIFAR100_resnet20_add/weights_best_240.pth')
partial = pretrained_model['state_dict']
weights = []
...
weights.append(partial['module.layer1.0.conv1.adder'].cpu().detach().numpy())
weights.append(partial['module.layer1.0.conv2.adder'].cpu().detach().numpy())
weights.append(partial['module.layer1.1.conv1.adder'].cpu().detach().numpy())
weights.append(partial['module.layer1.1.conv2.adder'].cpu().detach().numpy())
weights.append(partial['module.layer3.0.conv1.adder'].cpu().detach().numpy())
weights.append(partial['module.layer3.0.conv2.adder'].cpu().detach().numpy())
weights.append(partial['module.layer3.1.conv1.adder'].cpu().detach().numpy())
weights.append(partial['module.layer3.1.conv2.adder'].cpu().detach().numpy())
...
for j in range(8):
    for i in range (1):
        w_flatten.append(np.reshape(weights[1*j+i], [-1]))
        plot_weights.append(np.concatenate([all_weights, w_flatten[1*j+i]], axis=0))
        ax[j].hist(plot_weights[1*j+i], bins=100, color="b",range=[-5,5])
        # ax[j][i].xaxis.set_major_locator(ticker.MultipleLocator(1))
        ax[j].set_title(text[i])  
# plt.xticks(rotation=15)
plt.savefig("visualize/resnet20_add_distribution.pdf")
plt.clf()
